...
document.querySelector("#shutter").addEventListener(
	...
	() => {
		...
		cocoSsd.load().then(async (model) => {
			...
			// tensorflowの判定データの数
			console.log(tensorflow_judge_date.length);
			// 一番、正しい可能性が高いデータ＝先頭データ
			console.log(tensorflow_judge_date[0]);
			// tensorflowが何と判定したか
			console.log(tensorflow_judge_date[0].class);
			// 写真のどの範囲で判定したかのデータ
			console.log(tensorflow_judge_date[0].bbox);
			const tensorflow_judge_date_class_array = [];
			// tensorflowの判定データの数だけ繰り返す
			for (i = 0; i < tensorflow_judge_date.length; ++i) {
				// alert(i);
				tensorflow_judge_date_class_array.push(`
					<p>
            ${tensorflow_judge_date[i].class}
          </p>
					<canvas id="canvas_loop${i}">
					</canvas>
				`);
				// htmlのcanvas_loopにtensorflow_judge_date_class_arrayを出す。
				$("#canvas_loop"+i).html(tensorflow_judge_date_class_array);
			}
			...
			// tensorflow_judge_date_canvasのcanvasをIMGに変換。
